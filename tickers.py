...
def manage_tickers(bot, message):
    ...
    bot.send_message(config.ALARM_CHAT_ID, "Выберите действие:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)

def initiate_add_ticker(bot, call):
    markup = types.InlineKeyboardMarkup()
    bot.answer_callback_query(call.id)
    msg = bot.send_message(config.ALARM_CHAT_ID, "Введите имя тикера:", message_thread_id=config.ALARM_THEME_ID)
...
def ask_for_exchange(message, bot):
    ...
    markup.add(types.InlineKeyboardButton("Отмена", callback_data="cancel_add_ticker"))
    bot.send_message(config.ALARM_CHAT_ID, "Выберите биржу:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)

def handle_exchange_selection(bot, call):
    parts = call.data.split('_', 2)
    if len(parts) < 3:
        bot.send_message(config.ALARM_CHAT_ID, "Ошибка в данных. Пожалуйста, попробуйте снова.", message_thread_id=config.ALARM_THEME_ID)
        return
    ...
    if current_rate is None:
        bot.send_message(config.ALARM_CHAT_ID, "Не удалось получить текущую цену тикера, попробуйте другую биржу.", message_thread_id=config.ALARM_THEME_ID)
        return
    bot.send_message(config.ALARM_CHAT_ID, f"Текущая цена {ticker_name} на {exchange}: {current_rate}", message_thread_id=config.ALARM_THEME_ID)
...
def ask_for_direction(bot, message, ticker_name, exchange, current_rate):
    ...
    bot.send_message(config.ALARM_CHAT_ID, "Выберите направление сделки:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
...
def process_direction(bot, call):
    ...
    try:
        ...
        bot.send_message(config.ALARM_CHAT_ID, "Ошибка при конвертации текущего курса в число.", message_thread_id=config.ALARM_THEME_ID)
        return
    markup.add(types.InlineKeyboardButton("Отмена", callback_data="cancel_add_ticker"))
    bot.send_message(config.ALARM_CHAT_ID, f"Введите точку входа для {ticker_name} ({direction}):", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
...
def process_entry_point(message, bot, ticker_name, exchange, direction, current_rate):
    ...
    try:
        ...
        bot.send_message(config.ALARM_CHAT_ID, "Пожалуйста, введите корректное число для точки входа.", message_thread_id=config.ALARM_THEME_ID)
        return
    markup.add(types.InlineKeyboardButton("Отмена", callback_data="cancel_add_ticker"))
    bot.send_message(config.ALARM_CHAT_ID, "Введите значение тейк-профит:", message_thread_id=config.ALARM_THEME_ID)
...
def process_take_profit(message, bot, ticker_name, exchange, direction, entry_point, current_rate):
    ...
    try:
        ...
        bot.send_message(config.ALARM_CHAT_ID, "Пожалуйста, введите корректное число для тейк-профит.", message_thread_id=config.ALARM_THEME_ID)
        return
    markup.add(types.InlineKeyboardButton("Отмена", callback_data="cancel_add_ticker"))
    bot.send_message(config.ALARM_CHAT_ID, "Введите значение стоп-лосс:", message_thread_id=config.ALARM_THEME_ID)
...
def process_stop_loss(message, bot, ticker_name, exchange, direction, entry_point, take_profit, current_rate):
    ...
    try:
        ...
        bot.send_message(config.ALARM_CHAT_ID, "Пожалуйста, введите корректное число для стоп-лосс.", message_thread_id=config.ALARM_THEME_ID)
        return
    markup.add(types.InlineKeyboardButton("Отмена", callback_data="cancel_add_ticker"))
    bot.send_message(config.ALARM_CHAT_ID, "Прикрепите изображение сетапа или отправьте URL:", message_thread_id=config.ALARM_THEME_ID)
...
def finalize_setup(message, bot, ticker_name, exchange, direction, entry_point, take_profit, stop_loss, current_rate):
    ...
    try:
        ...
        bot.send_message(config.ALARM_CHAT_ID, "Тикер успешно добавлен в ваш портфель!", message_thread_id=config.ALARM_THEME_ID)
    except Exception as e:
        bot.send_message(config.ALARM_CHAT_ID, f"Ошибка при добавлении данных: {e}", message_thread_id=config.ALARM_THEME_ID)
...
def show_ticker_list(bot, message):
    ...
    bot.send_message(config.ALARM_CHAT_ID, "Выберите тикер:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
    ...
    try:
        ...
        if ticker:
            ...
            if ticker[6] and os.path.exists(ticker[6]):
                bot.send_photo(config.ALARM_CHAT_ID, open(ticker[6], 'rb'), message_thread_id=config.ALARM_THEME_ID)
    except Exception as e:
        bot.send_message(config.ALARM_CHAT_ID, f"Failed to create chart: {str(e)}", message_thread_id=config.ALARM_THEME_ID)
    finally:
        cursor.close()
        connection.close()
...
def delete_ticker(bot, call):
    ...
    bot.send_message(config.ALARM_CHAT_ID, "Выберите тикер для удаления:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)

def confirm_delete_ticker(bot, call):
    parts = call.data.split("_")
    if len(parts) < 2:
        bot.send_message(config.ALARM_CHAT_ID, "Произошла ошибка при обработке вашего запроса.", message_thread_id=config.ALARM_THEME_ID)
        return
    ticker_id = int(parts[1])
    db.delete_ticker(ticker_id)
    bot.answer_callback_query(call.id, "Тикер удален!")
    bot.edit_message_text("Тикер успешно удален.", config.ALARM_CHAT_ID, call.message.message_id, message_thread_id=config.ALARM_THEME_ID)
...
def edit_ticker(bot, call):
    ...
    bot.send_message(config.ALARM_CHAT_ID, "Выберите тикер для редактирования:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
...
def select_field_to_edit(bot, call):
    ...
    bot.send_message(config.ALARM_CHAT_ID, "Выберите поле для редактирования:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
...
def get_new_value(bot, call):
    ...
    bot.send_message(config.ALARM_CHAT_ID, msg, reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
    bot.register_next_step_handler_by_chat_id(config.ALARM_CHAT_ID, lambda message: update_ticker_value(bot, message, ticker_id, field))

def update_ticker_value(bot, message, ticker_id, field):
    new_value = message.text
    try:
        db.update_ticker_field(ticker_id, field, new_value)
        bot.send_message(config.ALARM_CHAT_ID, f"Значение для {field.replace('_', ' ').title()} успешно обновлено!", message_thread_id=config.ALARM_THEME_ID)
    except Exception as e:
        bot.send_message(config.ALARM_CHAT_ID, f"Ошибка при обновлении данных: {e}", message_thread_id=config.ALARM_THEME_ID)
...
def send_alert(ticker_id, message_text, reply_markup=None):
    ...
    try:
        if reply_markup:
            global_bot.send_message(chat_id=config.ALARM_CHAT_ID, text=message_text, reply_markup=reply_markup, message_thread_id=config.ALARM_THEME_ID)
        else:
            global_bot.send_message(chat_id=config.ALARM_CHAT_ID, text=message_text, message_thread_id=config.ALARM_THEME_ID)
        logging.info(f"Sent alert to {config.ALARM_CHAT_ID}: {message_text}")
    except Exception as e:
        logging.error(f"Failed to send alert to {config.ALARM_CHAT_ID}: {str(e)}")
...
def archive_tickers_list(bot, message):
    ...
    try:
        ...
        # Добавляем кнопки "Очистить архив" и "Выборочное удаление сделок"
        markup.add(types.InlineKeyboardButton("Очистить архив", callback_data="clear_all_archive"))
        markup.add(types.InlineKeyboardButton("Выборочное удаление сделок", callback_data="selective_delete_trades"))
        
        bot.send_message(config.ALARM_CHAT_ID, "Выберите сделку для просмотра:", reply_markup=markup, message_thread_id=config.ALARM_THEME_ID)
    except mysql.connector.Error as e:
        bot.send_message(config.ALARM_CHAT_ID, f"Ошибка при получении данных: {e}", message_thread_id=config.ALARM_THEME_ID)
    finally:
        cursor.close()
        connection.close()
